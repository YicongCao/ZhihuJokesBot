...
def get_history_joke_url_list():
    ...
    def parse_section_json(sec_html):
        ...
        return sec_json['timestamp'], joke_id_sub_list

    utils.clear_list_to_text('joke_id')
    joke_id_total_list = []
    timestamp, joke_id_list = parse_section_json(cur_section_html)
    ...
    while None != timestamp and len(joke_id_list) > 0:
        ...
        utils.save_list_to_text('joke_id', joke_id_list)
        # time.sleep(0.01)
...
if __name__ == "__main__":
    ...
    print(get_history_joke_url_list())
